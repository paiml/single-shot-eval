...
# Note: SEC010 ignored (scripts validate paths), DET002 ignored (timestamps intentional)
# SC2154 ignored (BASH_SOURCE/BASH_REMATCH are bash builtins)
lint-bash: ## Lint shell scripts and Makefile with bashrs
	@echo "Linting shell scripts with bashrs..."
	@find . -name "*.sh" -type f ! -path "./target/*" | while read -r script; do \
		echo "  $$script"; \
		bashrs lint --ignore SEC010,SC2035,SC2201,DET002,SC2154 --level error "$$script" || exit 1; \
	done
	@echo "Linting Makefile with bashrs..."
	bashrs make lint Makefile || true
	@echo "Bash linting complete (warnings reported, errors fail)"
...
# Run benchmarks and update README.md with results
bench-readme: ## Run benchmarks and update README.md table
	@echo "Running benchmarks and updating README.md..."
	./scripts/bench-update-readme.sh
...
# =============================================================================
# Evaluating CODE SLMs for Python→Rust single-shot compilation
...
# =============================================================================
# CODE-FOCUSED SLMs for Python→Rust Transpilation
# =============================================================================
...
models-download: ## Download code-focused SLMs (GGUF format)
	...
	@test -f models/raw/deepseek-coder-1.3b-instruct-q4_k_m.gguf || \
		(echo "  → DeepSeek-Coder-1.3B-Instruct Q4_K_M (~800MB)" && \
	...
	@test -f models/raw/starcoder2-3b-q4_k_m.gguf || \
		(echo "  → StarCoder2-3B Q4_K_M (~1.8GB)" && \
	...
	@echo ""
	@echo "✅ Raw models downloaded to models/raw/"
	@ls -lh models/raw/*.gguf

models-convert: ## Convert GGUF models to .apr format (entrenar)
	@echo "🔄 Converting GGUF → .apr format using entrenar..."
	@which entrenar > /dev/null 2>&1 || (echo "❌ entrenar not found. Install from batuta stack" && exit 1)
	@for gguf in models/raw/*.gguf; do \
		apr="models/$$(basename $$gguf .gguf).apr"; \
		if [ ! -f "$$apr" ]; then \
			echo "  Converting: $$gguf → $$apr"; \
			entrenar convert --input "$$gguf" --output "$$apr" || echo "  ⚠️  Failed: $$gguf"; \
		else \
			echo "  ✓ Already exists: $$apr"; \
		fi; \
	done
...
models-verify: ## Verify model checksums
	...
	@if [ -f $(MODEL_CHECKSUMS) ]; then \
		cd models && sha256sum -c CHECKSUMS.sha256 && echo "✅ Checksums verified"; \
	else \
		echo "⚠️  No checksums. Generating..."; \
...
corpus-check: ## Verify test corpus exists
	@echo "🔍 Checking test corpus: $(CORPUS_PATH)"
	@test -d $(CORPUS_PATH) || (echo "❌ Corpus not found: $(CORPUS_PATH)" && exit 1)
...
evaluate-quick: corpus-check ## Quick evaluation (10 examples)
	@echo "🔬 Quick evaluation (10 examples)..."
	$(CARGO) run --release -- evaluate \
		--corpus $(CORPUS_PATH) \
		--models models/*.apr \
		--samples 10 \
...
evaluate-offline: corpus-check ## Offline-only evaluation (no SaaS)
	@echo "🔬 OFFLINE evaluation (batuta stack only, no internet)..."
	@test -n "$$(ls models/*.apr 2>/dev/null)" || (echo "❌ No models. Run 'make models'" && exit 1)
	$(CARGO) run --release -- evaluate \
		--corpus $(CORPUS_PATH) \
		--models models/*.apr \
...
reproduce: models corpus-check evaluate report ## Full reproducible evaluation
	@echo "✅ Reproducible evaluation complete"
...
pmat-init:
	...
	@echo "✅ PMAT integration initialized"
...
help:
	...
	@echo ""
	@echo "🚀 QUICK START (Reproducible Science):"

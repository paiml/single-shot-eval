...
.PHONY: all check format lint test test-fast test-integration bench build build-release \
        run clean install help coverage coverage-html mutants ci \
        pmat-quality-gate pmat-rust-score pmat-validate-docs pmat-context \
        evaluate evaluate-quick report
...
# Run fast tests only (unit tests, <10s target)
test-fast:
	$(CARGO) test --lib
...
help:
	...
	@echo "  test-fast       - Run unit tests only (<10s)"
